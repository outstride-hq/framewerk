...
import type { ReasoningNode } from "./types.js";
...
function balancePersonas(personaIds: string[]): string[] {
  ...
  const challenging = nodes.filter(
    (n) => n.tags.some((t) => /challenge|risk|skeptical/i.test(t)) || n.biases?.includes("risk")
  );
  const supporting = nodes.filter(
    (n) => n.tags.some((t) => /support|optimistic|growth/i.test(t)) || n.biases?.includes("optimism")
  );
...
function pickDebateType(_frameworkIds: string[], personaIds: string[]): string {
  ...
  for (const dt of debateTypes) {
    ...
    if (outEdges.some((e) => personaSet.has(e.to))) return dt.id;
  }
